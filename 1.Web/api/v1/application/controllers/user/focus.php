...
class focus extends Base {
    ...
    public function lists(){
        ...
        do{
            ...
            $res = $this->modUser->getList('users_focus',$condition,'*','id DESC',$page,$size);
            if( $res->results ){
    ...
    public function add(){
    	...
    	do{
    		...
    		$res = $this->modUser->getList('users_focus',$condition,'*','id DESC',0,0);
    		if( count($res)==0 ){
    			//add user focus
    			$data = array(
    				'uid'			=> $uid,
    			...
    			$this->modUser->insert('users_focus',$data);
    		}
    ...
    public function remove(){
    	...
    	do{
    		...
    		$uid = $user['uid'];
    		//get focus
    		$single = $this->modUser->getSingle('users_focus','id',$params['id'],'uid');
    		if( !$single ){
    		...
    		//delete focus
    		$this->modUser->delete('users_focus',array('id'=>$params['id']),true);
    		$ret = array('err_no'=>0,'err_msg'=>'success','results'=>array());
    	} while(0);
    	$this->output($ret);
    }
