...
class seccode {
	...
	function ttffont() {
		...
		for($i = 0; $i < $seccodelength; $i++) {
			...
			$this->size && $font[$i]['size'] = mt_rand($font[$i]['size'] - $this->width / 50, $font[$i]['size'] + $this->width / 20);
    ...
    function flash() {
        ...
        if ($this -> adulterate) {
            $shape = new SWFShape();
            $shape -> setLine(3, rand(0, 255), rand(0, 255), rand(0, 255));
            $shape -> movePenTo(rand(0, $this -> width), rand(0, $this -> height));
            $shape -> drawCircle(rand(20, 31));
            $shape -> setLine(3, rand(0, 255), rand(0, 255), rand(0, 255));
            $shape -> movePenTo(rand(0, $this -> width), rand(0, $this -> height));
            $shape -> drawCircle(rand(15, 25));
            $shape -> movePenTo(rand(0, $this -> width), rand(0, $this -> height));
            $shape -> drawLine(rand(0, $this -> width), rand(0, $this -> height));
            $shape -> movePenTo(rand(0, $this -> width), rand(0, $this -> height));
            $shape -> drawLine(rand(0, $this -> width), rand(0, $this -> height));
            $movie -> add($shape);
        } 
