...
$seccodedata['adulterate'] = 0;
$seccodedata['ttf'] = 1;
$seccodedata['angle'] = 0;
$seccodedata['color'] = 0;
$seccodedata['size'] = 0;
$seccodedata['shadow'] = 0;
$seccodedata['animator'] = 0;
$timestamp = time();
$_DCOOKIE = array();
$seccode = 0;
$charset = "GBK";
...
function formcheck( $verifyName, $secc, $renew=true){
	$seccode = @$_SESSION[$verifyName];
	seccodeconvert($seccode);
	$tmp = substr($seccode, 0, 1);	
	if( strtoupper($secc)!=$seccode ) {
		return false;
    }
	$renew AND $_SESSION[$verifyName] = $seccode = random(6, 1) + $tmp * 1000000;
	return true;
}
