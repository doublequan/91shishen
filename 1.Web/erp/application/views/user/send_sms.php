        ...
        <form action="<?php echo site_url('user/send/doAddSMS'); ?>" method="post" id="dispatch_form">
            <div class="form-horizontal">
                ...
                <div class="form-group">
                    ...
                    <div class="col-sm-2 form_input">
                        <select name="template_id" class="form-control input-sm input-sm" id="template_id">
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-5 form_input">
                        <input type="text" name="content" class="form-control" value="" id="content">
                    </div>
                </div>
...
$(document).ready(function($) {
    ...
    $('#submit_btn').click( function(){
        ...
        if( content.length==0 ){
            $('#danger_alert').empty().text('请填写短信内容').show(500);
            return false;
        }
        $('#danger_alert').empty().hide();
        //send request
        $.ajax({
            type: "POST",
            url:  "<?php echo site_url('user/send/doSendSMS'); ?>",
            data: {'type':type,'group_id':group_id,'uids':uids.toString(),'template_id':template_id,'content':content},
            dataType: "json",
            success: function(msg){
                if( msg.err_no==0 ){
                    $('#submit_btn').addClass('disabled');
                    $('#success_alert').empty().text('群发短信成功').show(500);
                } else {
