...
$(document).ready(function($) {
    ...
    $('#submit_btn').click( function(){
        ...
        var title = $('#title').val();
        if( title.length==0 ){
            $('#danger_alert').empty().text('请填写消息标题').show(500);
            return false;
        }
        $('#danger_alert').empty().hide();
        //get content
        ...
        if( content.length==0 ){
            $('#danger_alert').empty().text('请填写消息内容').show(500);
            return false;
        }
        $('#danger_alert').empty().hide();
        //send request
        $.ajax({
            type: "POST",
            url:  "<?php echo site_url('user/send/doSendMsg'); ?>",
            data: {'type':type,'group_id':group_id,'uids':uids.toString(),'title':title,'content':content},
            dataType: "json",
            success: function(msg){
                if( msg.err_no==0 ){
                    $('#submit_btn').addClass('disabled');
                    $('#success_alert').empty().text('群发站内消息成功').show(500);
                } else {
