...
$page['title'] = '订单详情';
$order_id = isset($_GET['order_id']) ? base64_encode($_GET['order_id']) : 0;
...
<div class="pay_show" data-order-id="<?=$order_id?>">
	...
	<div class="pay_show-btn">
		...
		<a href="javascript:;" id="btn-cancel" data class="btn btn-red padding"><i class="icon-shop"></i> 取消订单</a>
	</div>
		...
		<ul class="clearfix">
			...
			{{# } }}
				...
				{{# if( d.delivery_type == 1) { }}
					<dl>
						<dt>收货地址：<label>{{ d.prov }}</label> &frasl; <label>{{ d.city }}</label> &frasl; <label>{{ d.district }}</label></dt>
				...
				<li>抵用券金额：<label>{{ d.price_discount }}</label> </li>
				<li>优惠金额：<label>{{ d.price_minus }}</label> </li>
				<li>代金券金额：<label>{{ d.price_cash }}</label> </li>
