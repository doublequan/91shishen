		...
		<div class="address">
			...
			<div class="addlist">
				...
				<?php if(!empty($data)):foreach($data as $v):?>
					<tr id="address_<?=$v['id']?>">
						<td><?=$v['receiver']?> 收<br><?=$v['prov_name']?>&nbsp;&nbsp;<?=$v['city_name']?>&nbsp;&nbsp;<?=$v['district_name']?>&nbsp;&nbsp;<?=$v['street_name']?>&nbsp;&nbsp;<?=$v['address']?><br><?=$v['mobile']?></td>
						<td class="w180">
							<div class="edit">
								<p>
			...
			<div class="add" id="add_address" style="display:none;">
				<ul>
					...
					<li><label>省市：</label>
						...
						<select id="area_id" onchange="getStreet()">
					...
					<li><label>固定电话：</label><input type="text" class="dh" id="tel" maxlength="13" /><span id="tip_tel" style="display:none;"></span></li>
...
function doAddAddress(address_id){
	...
	if(address_id == undefined){
		//添加
		var action_type = "添加";
		var post_url = "<?=base_url('member/address/ajax_add')?>";
		var post_data = {"receiver":receiver, "prov":province_id, "city":city_id, "district":area_id, "street":street_id, "address":address, "zip":zip, "tel":tel, "mobile":mobile, "is_default":is_default};
	}else{
		//更新
		var action_type = "编辑";
		var post_url = "<?=base_url('member/address/ajax_upd')?>";
		var post_data = {"id":address_id, "receiver":receiver, "prov":province_id, "city":city_id, "district":area_id, "street":street_id, "address":address, "zip":zip, "tel":tel, "mobile":mobile, "is_default":is_default};
	}
	layer.load("提交中...");
	$.ajax({
		url: post_url,
...
function setDefault(address_id){
	layer.load("提交中...");
	$.ajax({
		url: "<?=base_url('member/address/ajax_set')?>",
		type: "post",
		dataType: "json",
...
function updAddress(address_id){
	$.ajax({
		url: "<?=base_url('member/address/ajax_get')?>",
		type: "post",
		dataType: "json",
		data: {"address_id":address_id},
		success: function(data){
...
function delAddress(address_id){
	layer.confirm("确定删除？", function(){
		layer.load("提交中...");
		$.ajax({
			url: "<?=base_url('member/address/ajax_del')?>",
			type: "post",
			dataType: "json",
			data: {"address_id":address_id},
			success: function(data){
