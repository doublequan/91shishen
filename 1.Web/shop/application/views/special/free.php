    	...
    	<li class="fl">
        	...
        	<a href="<?php echo base_url("goods_{$row['id']}.html"); ?>" target="_black">
              <div>
                ...
                <span class="nn_link fr addCart buyed" disabled="disabled" product_id="<?php echo $row['id']; ?>">已抢购！</span>
                <?php } else { ?>
	...
	$('.addCart').click( function(){
		...
		if( buyedNum>=2 ){
			$.layer({
				...
				area: ['480px','130px'],
				dialog: {
					msg: '免费商品每天限购两款，多出商品将会以原价结算，是否继续购买？',
					btns: 2,                    
					type: -1,
					btn: ['原价购买','再看看'],
					yes: function(){
			...
			addCart(product_id);
			buyedNum++;
			$(this).html('已抢购！').addClass('buyed').attr('disabled','disabled');
		}
		return false;
	});
});
